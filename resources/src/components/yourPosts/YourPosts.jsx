...
const YourPosts = () => {
  ...
  const getUsersPosts = async () => {
    try {
      const response = await axios.get(
        `http://${host}:${port}/api/users-posts`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const deletePost = async (postId) => {
    ...
    if (postId == null) {
      ...
      const deleteOrNot = window.confirm(
        "Are you sure you want to delete this post?",
      );
      if (deleteOrNot) {
        try {
          await axios.post(`http://${host}:${port}/api/delete-posts/`, id, {
            headers: { Authorization: `Bearer ${token}` },
          });
          getUsersPosts();
        } catch (error) {
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 ">
            ...
            <tbody>
              {postData.length > 0 ? (
                ...
                <td className="px-6 py-4">You haven't upload any post</td>
              )}
            </tbody>
          </table>
        </div>
